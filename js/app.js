...
function lessContent(){
  less.innerHTML = less.style.display ="none";
}
...
var menuItems = ["McChicken", "McDouble", "Large Drink"];
...
Benjamin.addEventListener("click", display);
...
random.addEventListener("click", daQuotes);

var randomQuotes = quotes[Math.floor(Math.random() * quotes.length) ]; // math floor rounds to nearest interger
                                                                      // math random randomize between 0,1
function daQuotes(){
...
showHide.addEventListener("click", aloha);
